    ...
    while len(players) > 0:
        ...
        while strike_counts[current_player] < 3:
            ...
            if result == "스트라이크" or result == "헛스윙":
                ...
                if strike_counts[current_player] >= 3 or foul_counts[current_player] >= 4:
                    try:
                        ...
                        print(f"{current_player} - 아웃되었습니다.")
                    except ValueError:
                        print(f"{current_player} - 이미 아웃되었습니다.")
                    print('다음타자 등장 -----------------')
                    # 스트라이크, 헛스윙 카운트 초기화
            ...
            # 아웃인 경우
                ...
                print(f"{current_player}가 {hit_result} 했습니다.")
                print("스트라이크", strike_counts[current_player])
                print('파울', foul_counts[current_player])
                print('다음공 -----------------')
                ...
                if hit_result == "1루타":
                    ...
                    if base2 is not None:
                        print(f"{current_player.name} - 이미 2루에 선수가 있습니다. 다음 타자 등장 ->-<->-<-------------")
                        # 스트라이크, 헛스윙 카운트 초기화
                        strike_counts[current_player] = 0
                        foul_counts[current_player] = 0
                        break
